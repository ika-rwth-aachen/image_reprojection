...
ImageReprojection::ImageReprojection(const rclcpp::NodeOptions& options) : Node("image_reprojection", options) {
...
template <typename T>
void ImageReprojection::declareAndLoadParameter(const std::string& name,
                                                         T& param,
...
rcl_interfaces::msg::SetParametersResult ImageReprojection::parametersCallback(const std::vector<rclcpp::Parameter>& parameters) {
...
void ImageReprojection::setup() {

  // callback for dynamic parameter configuration
  parameters_callback_ = this->add_on_set_parameters_callback(std::bind(&ImageReprojection::parametersCallback, this, std::placeholders::_1));

  // subscriber for handling incoming messages
  subscriber_ = this->create_subscription<std_msgs::msg::Int32>("~/input", 10, std::bind(&ImageReprojection::topicCallback, this, std::placeholders::_1));
...
void ImageReprojection::topicCallback(const std_msgs::msg::Int32::ConstSharedPtr& msg) {
